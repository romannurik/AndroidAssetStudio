...
import { NinePatchTrimming } from './nine-patch-trimming';
...
export class NinePatchStage {
  ...
  setupUi() {
    // Stage code
    this.$topLabel = $('<div>')
      .addClass('canvas-label label-vertical')
      .hide()
      .appendTo('body');
    this.$leftLabel = $('<div>')
      .addClass('canvas-label label-horizontal')
      .hide()
      .appendTo('body');
    this.$rightLabel = $('<div>')
      .addClass('canvas-label label-horizontal')
    ...
    $('.trim-stretch-button').click(() =>
      NinePatchTrimming.trimStretchRegion(this)
    );
  ...
  setupDragging() {
    ...
    draggingMouseMoveHandler_ = ev => {
      ...
      if (this.editLeft) {
        let newX = Math.max(
          0,
          Math.min(editRect.x + editRect.w - 1, Math.round(offsetX / this.zoom))
        );
      ...
      if (this.editTop) {
        let newY = Math.max(
          0,
          Math.min(editRect.y + editRect.h - 1, Math.round(offsetY / this.zoom))
        );
      ...
      if (this.editRight) {
        editRect.w = Math.min(
          this.srcSize.w - editRect.x,
          Math.max(1, Math.round(offsetX / this.zoom) - editRect.x)
        );
      }
      if (this.editBottom) {
        editRect.h = Math.min(
          this.srcSize.h - editRect.y,
  ...
  redrawOverlay() {
    ...
    if (editRect === this.stretchRect) {
      ...
      ctx.moveTo(0, editRect.y * this.zoom + 0.5);
      ctx.lineTo(this.zoomedSize.w, editRect.y * this.zoom + 0.5);

      ctx.moveTo(0, (editRect.y + editRect.h) * this.zoom - 0.5);
      ctx.lineTo(
        this.zoomedSize.w,
        (editRect.y + editRect.h) * this.zoom - 0.5
      );

      ctx.moveTo(editRect.x * this.zoom + 0.5, 0);
      ctx.lineTo(editRect.x * this.zoom + 0.5, this.zoomedSize.h);

      ctx.moveTo((editRect.x + editRect.w) * this.zoom - 0.5, 0);
      ctx.lineTo(
        (editRect.x + editRect.w) * this.zoom - 0.5,
        this.zoomedSize.h
      );
    } else {
      ctx.beginPath();
      ctx.rect(
        editRect.x * this.zoom + 0.5,
        editRect.y * this.zoom + 0.5,
        editRect.w * this.zoom - 1,
        editRect.h * this.zoom - 1
      );
      ctx.closePath();
    }
  ...
  loadLastRects() {
    try {
      ...
      if (store.stretchRect && store.contentRect && store.opticalBoundsRect) {
        ...
        this.opticalBoundsRect = fitRect_(
          store.opticalBoundsRect,
          this.srcSize
        );
      }
    } catch (e) {}
